...
export function DetailPage({ detail, back, backLabel, legal = false }: { detail: Detail; back: string; backLabel: string; legal?: boolean }) {
  return <><Header /><main>
    <section className="detail-hero">
      <Link className="text-link" href={back}>← {backLabel}</Link>
    ...
    {!legal && <section className="detail-cta"><div><p className="eyebrow">Your next move</p><h2>Let’s put it to work.</h2></div><Link className="button" href={`/contact?interest=${encodeURIComponent(detail.title)}`}>Discuss your project <ArrowUpRight size={18} /></Link></section>}
  </main><Footer /></>;
}
